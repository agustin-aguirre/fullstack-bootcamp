...
function jokeExists(id) {
  return jokes.filter(j => j.id === id).length > 0;
}


//1. GET a random joke
app.get("/random", (req, res) => {
  let joke = jokes[Math.floor(Math.random(0, jokes.length))];
  res.send(joke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filteredJokes = jokes.filter((joke) => joke.jokeType === filterType);
...
app.post("/jokes", (req, res) => {
  const joke = req.body;
  if (!joke.text || !joke.type) res.sendStatus(400);
  const newJoke = new Joke(joke.text, joke.type);
  newJoke.id = jokes.length;
  jokes.push(joke);
  res.send(newJoke);
});
...
app.put("/jokes/:id", (req, res) => {
  if (!jokeExists(req.params.id)) res.sendStatus(404);
  const joke = req.body;
  if (!joke.text || !joke.type ) res.sendStatus(400);
  const newJoke = new Joke(joke.text, joke.type);
  newJoke.id = req.params.id;
  jokes[newJoke.id] = newJoke.id
  res.send(newJoke);
});
